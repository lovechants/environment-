...
class AttentionKANLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        # Sum the psi outputs
        return torch.sum(psi_outputs, dim=1), attention_weights
...
class AttentionKAN(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.final_layer(x), attention_weights
...
for epoch in range(num_epochs):
    ...
    output_attention, attention_weights = model_attention(x)
...
def plot_attention_weights(attention_weights):
    ...
    # Average attention weights across the batch
    avg_attention_weights = attention_weights.mean(dim=0).squeeze().cpu().numpy()
    
    # Ensure avg_attention_weights is 2D
    if avg_attention_weights.ndim == 1:
        avg_attention_weights = avg_attention_weights[None, :]  # Add an extra dimension to make it 2D
    
    # Normalize the data to be between 0 and 1
    avg_attention_weights = (avg_attention_weights - avg_attention_weights.min()) / (avg_attention_weights.max() - avg_attention_weights.min())
    
    # Plot the heatmap
    heatmap = plt.imshow(avg_attention_weights, cmap='viridis', aspect='auto')
    ...
    for i in range(avg_attention_weights.shape[0]):
        for j in range(avg_attention_weights.shape[1]):
            text = plt.text(j, i, f"{avg_attention_weights[i, j]:.2f}",
                            ha="center", va="center", color="w" if avg_attention_weights[i, j] < 0.5 else "k")
